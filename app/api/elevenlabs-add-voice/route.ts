...
// Initialize the Bytescale Upload Manager
...
export async function POST(req: NextRequest) {
  ...
  const sourceText = body.sourceText; // Text to be converted to speech
  const sourceVid = body.voiceFile; // Video file path
  const voiceName = body.voiceName; // Name for the new voice
  const description = body.description; // Description for the new voice

  // Initialize Bytescale File API
  const fileApi = new Bytescale.FileApi({
    fetchApi: nodeFetch as any,
    apiKey: "public_12a1yvy634kYX3ss1W9DgV64CaeC",
  });

  // Initialize Bytescale Job API
  ...
  // Process the video file to convert it to MP3
  ...
  // Mp3 file after vidInput function
  ...
  // Function to create an audio stream from text using Eleven Labs
  ...
  // Audio stream from eleven labs 
  const stream = await createAudioStreamFromText(sourceText);

  // upload to bytescale
  const path = await uploadManager
    .upload({
      data: stream,
